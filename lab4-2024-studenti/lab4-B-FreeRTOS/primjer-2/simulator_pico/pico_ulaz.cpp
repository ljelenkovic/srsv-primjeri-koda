...
void init_gpio_in(int port){
    m_pinNumber_in = port;
    gpio_init(m_pinNumber_in);
    gpio_set_dir(m_pinNumber_in, GPIO_IN);
    gpio_pull_up(m_pinNumber_in);
}

void init_gpio_out(int port)
{   
    m_pinNumber_in = port;
    gpio_init(m_pinNumber_in);
    gpio_set_dir(m_pinNumber_in, GPIO_OUT);
    }
...
void inicijalizacija(){
    ...
    while(gpio_get(m_pinNumber_in) == HIGH){
...
void simulacija_ulaza(int i){
       if(ulaz[i].x == 0){
        ...
        while(!kraj_simulacije){
            ...
            promjena[i] = false;
            gpio_put(m_pinNumber_out, HIGH);
            if(gpio_get(m_pinNumber_in) == LOW){
                gotovo[i] = true;
            }
